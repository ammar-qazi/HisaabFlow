...
class DataCleaner:
    ...
    def _standardize_columns(self, data: List[Dict], template_config: Dict = None) -> Tuple[List[Dict], Dict[str, str]]:
        ...
        # FIRST: Strip BOM characters from all column names (Universal Fix)
        bom_cleaned_data = self._strip_bom_from_columns(data)
        data = bom_cleaned_data
    ...
    def _strip_bom_from_columns(self, data: List[Dict]) -> List[Dict]:
        """
        Universal fix: Remove BOM characters from column names
        This fixes Wise CSV files that have \ufeff at the start of column names
        """
        if not data:
            return []
        ...
        sample_row = data[0]
        has_bom = any('\ufeff' in str(col) for col in sample_row.keys())
    ...
    def _remove_invalid_rows(self, data: List[Dict]) -> List[Dict]:
        ...
        # NEW: Add data quality check
        if valid_data:
            self._check_data_quality(valid_data)
        
        return valid_data
    
    def _check_data_quality(self, data: List[Dict]) -> None:
        """
        ...
        for col in essential_cols:
            if col in sample_row:
                ...
                if empty_count > 0:
                    quality_issues.append(f"{empty_count}/{len(data)} rows have empty '{col}' values")
        
        # Check for potential BOM issues (should be fixed by now)
        bom_cols = [col for col in sample_row.keys() if '\ufeff' in str(col)]
        if bom_cols:
            quality_issues.append(f"BOM characters still present in columns: {bom_cols}")
        ...
        if 'Amount' in sample_row:
            ...
            if non_numeric_amounts > 0:
                quality_issues.append(f"{non_numeric_amounts}/{len(data)} amounts are not numeric")
    ...
    def _create_updated_column_mapping(self, template_config: Dict = None, column_name_mapping: Dict = None) -> Dict[str, str]:
        ...
        # NEW: Validate column mapping against actual data
        self._validate_column_mapping(updated_mapping, template_config)
        
        print(f"      ✅ Updated mapping created: {updated_mapping}")
        return updated_mapping
    
    def _validate_column_mapping(self, column_mapping: Dict[str, str], template_config: Dict = None) -> None:
        """
        Validate that column mappings match actual data structure
        Warns about potential mapping mismatches that could cause data loss
        """
        if not template_config:
            return
        
        bank_name = template_config.get('bank_name', 'Unknown')
        print(f"      🔍 Validating column mapping for {bank_name}...")
        
        # Check for common mapping issues
        validation_warnings = []
        
        # Check for missing essential mappings
        essential_cols = ['Date', 'Amount', 'Title', 'Note']
        for col in essential_cols:
            mapped_col = column_mapping.get(col, '')
            if not mapped_col or mapped_col == '':
                validation_warnings.append(f"Missing mapping for essential column '{col}'")
        
        # Bank-specific validation rules
        if 'wise' in bank_name.lower():
            # Wise-specific validations
            title_mapping = column_mapping.get('Title', '')
            note_mapping = column_mapping.get('Note', '')
            
            if title_mapping != 'Description':
                validation_warnings.append(f"Wise banks: Title should map to 'Description', found '{title_mapping}'")
            
            if note_mapping not in ['PaymentReference', 'Payment Reference']:
                validation_warnings.append(f"Wise banks: Note should map to 'PaymentReference', found '{note_mapping}'")
        
        elif 'nayapay' in bank_name.lower():
            # NayaPay-specific validations
            title_mapping = column_mapping.get('Title', '')
            if title_mapping not in ['DESCRIPTION', 'Description']:
                validation_warnings.append(f"NayaPay: Title should map to 'DESCRIPTION', found '{title_mapping}'")
        
        # Report validation results
        if validation_warnings:
            print(f"      ⚠️ Column mapping validation warnings:")
            for warning in validation_warnings:
                print(f"         • {warning}")
        else:
            print(f"      ✅ Column mapping validation passed")
    ...
    def get_bank_specific_cleaning_rules(self, bank_name: str) -> Dict[str, Any]:
        """
        NEW: Get bank-specific data cleaning rules
        This allows different banks to have different cleaning approaches
        """
    ...
    template_config = {
        ...
        'bank_name': 'NayaPay'
            ...
            'nayapay': {
                'strip_bom': False,
                'date_formats': ['%d %b %Y %I:%M %p', '%Y-%m-%d'],
    ...
    print(json.dumps(result, indent=2, default=str))
                ...
                'required_columns': ['TIMESTAMP', 'AMOUNT', 'DESCRIPTION'],
                'optional_columns': ['TYPE', 'BALANCE']
            }
        }
        
        # Find matching bank rules
        for bank_key, rules in cleaning_rules.items():
            if bank_key in bank_name_lower:
                print(f"      🔧 Using {bank_key} specific cleaning rules")
                return rules
        
        # Default rules
        return {
            'strip_bom': True,  # Enable BOM stripping by default
            'date_formats': ['%Y-%m-%d'],
            'amount_patterns': [r'^[+-]?\d+\.\d{2}
    cleaner = DataCleaner()
